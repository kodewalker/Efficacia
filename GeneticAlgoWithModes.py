...
class GeneticAlgorithmTSP:
    ...
    def __init__(
        ...
        self.air_cost_mat = air_cost_mat
        self.train_cost_mat = train_cost_mat
...
#         my_randoms_mode = [random.sample(range(1,4),1)[0],random.sample(range(1,4),1)[0],random.sample(range(1,4),1)  [0],random.sample(range(1,4),1)[0],random.sample(range(1,4),1)[0]]
    ...
    def fitness_function(self, route) -> int:
        ...
        Implementation:
                ...
                output: 17514
        """
        air_data = self.air_cost_mat.copy()
        train_data = self.train_cost_mat.copy()
        ...
        while stops_covered < len(route1):
            minimum_cost = 0
            a=traverse_data.iloc[source_point, route1[stops_covered]]
            b=air_data.iloc[source_point, route1[stops_covered]]
            c=train_data.iloc[source_point, route1[stops_covered]]
            ...
            route_cost = route_cost + minimum_cost
            mode=mode+","+mode1
#             print("mode",mode1,source_point,route1[stops_covered])
...
#         a=traverse_data.iloc[route1[-1], 0]
#         b=air_data.iloc[route1[-1], 0]
#         c=train_data.iloc[route1[-1], 0]
...
class OverallGaRun(GeneticAlgorithmTSP):
    ...
    def run_overall_ga(self):
        ...
        final_route = [starting_point] + final_route
